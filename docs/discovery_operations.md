...
3. Enable the new workflows in GitHub Actions:
   ...
   - `news-items-backfill-discover`
   - `news-items-backfill-scrape`
4. Confirm `daily-state-run` and `weekly-state-run` run the shared base config
   `agents/news/local_search_brave_exa.yaml` with `--overlay agents/news/ci.overlay.yaml`.
