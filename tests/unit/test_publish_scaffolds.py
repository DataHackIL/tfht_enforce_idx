...
class DescribedReleaseBuilder(ReleaseBuilder):
...
class DescribedBackupExecutor(BackupExecutor):
...
def test_release_builder_base_raises_not_implemented() -> None:
    """The abstract release builder contract should remain explicit."""
    with pytest.raises(NotImplementedError):
        DescribedReleaseBuilder().build_manifest("news_items", Path("/tmp/publication"))


def test_null_backup_executor_returns_placeholder_target() -> None:
    """The backup scaffold should return a placeholder local target."""
    manifest = NullBackupExecutor().build_manifest("news_items", Path("/tmp/state-root"))

    assert manifest.dataset_name == "news_items"
    assert len(manifest.targets) == 1
    assert manifest.targets[0].location == "/tmp/state-root"
...
def test_backup_executor_base_raises_not_implemented() -> None:
    """The abstract backup executor contract should remain explicit."""
    with pytest.raises(NotImplementedError):
        DescribedBackupExecutor().build_manifest("news_items", Path("/tmp/state-root"))
