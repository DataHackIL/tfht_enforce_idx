...
- Next planned PR: run a bounded local or CI candidate-drain evidence pass under an ignored state
  root, then use its queue, source-health, scrape-failure, and backfill timing metrics to choose the
  next narrow source-health, backfill, or self-healing implementation PR.
...
- [next] Produce a bounded candidate-drain evidence bundle from a fresh local or CI run, including
  discovery queue health, source-health artifact output, scrape-failure groups, and any backfill
  status timing needed to choose the next narrow implementation PR.
