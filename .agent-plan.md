...
- Last merged PR on main: `SRC-PR-ISRAELHAYOM` added bounded generic-fetch source-family support
  for Israel Hayom: source-targeted discovery/backfill queries cover `www.israelhayom.co.il`,
  search-discovered candidates and fallback provenance group as `israelhayom`, and
  browser/source-native scraper work remains out of scope because the January evidence showed
  candidate-only backlog rather than attempted scrape or partial-page behavior.
...
- [done] `SRC-PR-ISRAELHAYOM`: add bounded generic-fetch source-family support for Israel Hayom,
  including source-targeted discovery/backfill queries, without adding a browser/source-native
  scraper.
