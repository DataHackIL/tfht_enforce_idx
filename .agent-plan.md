...
- Last merged PR on main: `UNIFY-PR-01` — local and GitHub Actions now share a single canonical
  news config, `agents/news/local_search_brave_exa.yaml`. The CI-only `agents/news/github.yaml`
  is removed; CI selects its deployment-specific behaviour at runtime via env overrides on the
  shared config (`DENBUST_OPERATIONAL_PROVIDER=supabase`, `DENBUST_OUTPUT_FORMATS=cli,email`),
  honoured by new before-validator overrides on `OperationalConfig` and `OutputConfig` in
  `config.py`. All seven operational workflows, the 2026 backfill helper script, and the
  operational/discovery docs point at the canonical config. This removes the config-drift risk
  between the two surfaces and is the first step of the local↔CI unification track.
...
- [done] `UNIFY-PR-01`: collapse local and GitHub Actions onto a single canonical news config.
  `agents/news/github.yaml` is removed; both surfaces run `agents/news/local_search_brave_exa.yaml`.
  CI selects the deployment-specific behaviour at runtime through env overrides instead of a second
  file: `OperationalConfig` honours `DENBUST_OPERATIONAL_PROVIDER` (CI sets `supabase`) and
  `OutputConfig` honours `DENBUST_OUTPUT_FORMATS` (CI sets `cli,email`), mirroring the existing
  before-validator env-override idiom in `config.py`. All seven operational workflows, the 2026
  backfill helper script, and the operational/discovery docs point at the canonical config; tests
  in `test_config.py` cover the env-driven provider/output flip. Eliminates the config-drift risk
