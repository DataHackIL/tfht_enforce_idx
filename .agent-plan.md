...
- Last merged PR on main: `CLASSIFIER-PR-PARSE-FAILURE-EVIDENCE-CAPTURE` persists sanitized
  classifier parse-failure shape diagnostics in full and compact run debug artifacts, including
  fallback-only scrape/backfill summaries, without storing raw classifier responses or changing
  parser recovery behavior.
- Next planned PR: run a bounded Phase C fallback classifier evidence pass that uses
  `classifier_summary.parse_failure_diagnostics` and
  `fallback_classifier_summary.parse_failure_diagnostics` to decide whether any parse-failure shape
  is safely recoverable, without changing classifier prompts, taxonomy policy, queue behavior,
  scraper behavior, scrape caps, source-family support, or committing generated artifacts.
...
- [done] `CLASSIFIER-PR-PARSE-FAILURE-EVIDENCE-CAPTURE`: persist sanitized classifier
  ...
  classifier prompts, taxonomy validity policy, queue behavior, scraper behavior, scrape caps, or
  source-family support.
- [next] `CLASSIFIER-PR-PARSE-FAILURE-EVIDENCE-INTERPRETATION`: run a bounded Phase C fallback
  classifier evidence pass and interpret the sanitized parse-failure shape diagnostics before any
  parser recovery work, without changing classifier prompts, taxonomy validity policy, legacy
  taxonomy policy, queue behavior, scraper behavior, scrape caps, source-family support, or
